...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // For each product card, set up event listeners
    ...
    function createProductElement(product, index) {
        ...
        priceP.textContent = `R${(product.numericPrice * product.quantity).toFixed(2)}`;
        ...
        upArrow.addEventListener('click', () => updateItemQuantity(index, 1));
    ...
    // Initialize cart checkout button
    const checkoutButton = document.querySelector('.cart-checkout-btn');
    checkoutButton.addEventListener('click', function() {
        alert('Proceeding to checkout with ' + cartItems.length + ' items totaling R' + calculateSubtotal().toFixed(2));
    ...
    // Fix for nested link in button
    const cartIconLink = cartIcon.querySelector('a');
    if (cartIconLink) {
